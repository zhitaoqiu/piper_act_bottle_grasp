#!/bin/bash
# Tiny ACT overfit — chunk_size=10, n_action_steps=10, smaller model.
# After chunk_size=1 succeeded, verify chunk_size=10 with same small architecture.
set -euo pipefail

DATASET_ROOT="${DATASET_ROOT:-data/lerobot_dataset_today_approach_1ep}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/train/piper_bottle_approach_tiny_c10_1ep}"
STEPS="${STEPS:-5000}"
...
PYTHONPATH= ~/miniconda3/envs/piper_act/bin/python3 -m lerobot.scripts.lerobot_train \
    ...
    --policy.repo_id=piper/bottle_approach_tiny_c10_1ep_overfit \
    ...
    --job_name=piper_act_approach_tiny_c10_1ep
