#!/bin/bash
# ACT Training for Piper Bottle Grasp, chunk_size=40 / n_action_steps=40

set -euo pipefail

DATASET_ROOT="${DATASET_ROOT:-data/lerobot_dataset}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/train/piper_bottle_grasp_chunk40}"
...
PYTHONPATH= ~/miniconda3/envs/piper_act/bin/python3 -m lerobot.scripts.lerobot_train \
    ...
    --policy.repo_id=piper/bottle_grasp_act_chunk40 \
